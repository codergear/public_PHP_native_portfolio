...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $mouthguards_models->models = $retriveData->models;
        $mouthguards_models->manufacturer = $retriveData->manufacturer;
     $OPCODE_RESULT = $mouthguards_models->insert($mouthguards_models,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_mouthguards_models = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'mouthguards_models', 'create', $retriveData->id_mouthguards_models);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $mouthguards_models = new classmouthguards_models();     
       if ($retriveData->id_mouthguards_models != 0){
           $mouthguards_models_list = $mouthguards_models->selectBy('id_mouthguards_models', $retriveData->id_mouthguards_models);
           $mouthguards_models_list = $mouthguards_models_list ? $mouthguards_models_list[0] : null ;
       }else{
           $mouthguards_models_list =  $mouthguards_models->selectAll();
       }
       $mouthguards_manufacturer=new classmouthguards_manufacturer();
       $mouthguards_manufacturer=$mouthguards_manufacturer->selectAll();
       if (($mouthguards_models_list) && ($retriveData->id_mouthguards_models == 0)){
           foreach ($mouthguards_models_list as $mouthguards_models_item){
               foreach ($mouthguards_manufacturer as $item){
                   if ($item->id_mouthguards_manufacturer == $mouthguards_models_item->manufacturer){
                       $mouthguards_models_item->_manufacturer = $item->manufacturer;
                   }
               }
               if (!isset($mouthguards_models_item->_manufacturer)){$mouthguards_models_item->_manufacturer='';}
           }
       } 
       if(!$mouthguards_models_list){
           $mouthguards_models_list = new classmouthguards_models;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'mouthguards_models', 'read', $retriveData->id_mouthguards_models);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"mouthguards_models":'. json_encode($mouthguards_models_list).',"mouthguards_manufacturer":'. json_encode($mouthguards_manufacturer).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    $mouthguards_models->models = $retriveData->models;
    $mouthguards_models->manufacturer = $retriveData->manufacturer;
    $mouthguards_models->id_mouthguards_models = $retriveData->id_mouthguards_models;
    $OPCODE_RESULT = $mouthguards_models->update($mouthguards_models,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'mouthguards_models', 'update', $retriveData->id_mouthguards_models);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $mouthguards_models = new classmouthguards_models;   
      $QUERY_RESULT = $mouthguards_models->delete($retriveData->id_mouthguards_models,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'mouthguards_models', 'delete', $retriveData->id_mouthguards_models);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
