...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $compensation_schedule->document_content = $retriveData->document_content;
     $OPCODE_RESULT = $compensation_schedule->insert($compensation_schedule,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_compensation_schedule = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'compensation_schedule', 'create', $retriveData->id_compensation_schedule);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $compensation_schedule = new classcompensation_schedule();     
       if ($retriveData->id_compensation_schedule != 0){
           $compensation_schedule_list = $compensation_schedule->selectBy('id_compensation_schedule', $retriveData->id_compensation_schedule);
           $compensation_schedule_list = $compensation_schedule_list ? $compensation_schedule_list[0] : null ;
       }else{
           $compensation_schedule_list =  $compensation_schedule->selectAll();
       }
       if(!$compensation_schedule_list){
           $compensation_schedule_list = new classcompensation_schedule;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'compensation_schedule', 'read', $retriveData->id_compensation_schedule);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"compensation_schedule":'. json_encode($compensation_schedule_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    $compensation_schedule = new classcompensation_schedule;   
    $compensation_schedule->document_content = $retriveData->document_content;
    $compensation_schedule->id_compensation_schedule = $retriveData->id_compensation_schedule;
    $OPCODE_RESULT = $compensation_schedule->update($compensation_schedule,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'compensation_schedule', 'update', $retriveData->id_compensation_schedule);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $compensation_schedule = new classcompensation_schedule;   
      $QUERY_RESULT = $compensation_schedule->delete($retriveData->id_compensation_schedule,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'compensation_schedule', 'delete', $retriveData->id_compensation_schedule);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
