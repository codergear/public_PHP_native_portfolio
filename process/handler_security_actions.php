...
function create($retriveData,$id_user) {
  ...
  try {
        $security_actions = new classsecurity_actions;
        $security_actions->action = $retriveData->action;
     $OPCODE_RESULT = $security_actions->insert($security_actions,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_security_actions = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'security_actions', 'create', $retriveData->id_security_actions);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $security_actions = new classsecurity_actions();     
       if ($retriveData->id_security_actions != 0){
           $security_actions_list = $security_actions->selectBy('id_security_actions', $retriveData->id_security_actions);
           $security_actions_list = $security_actions_list ? $security_actions_list[0] : null ;
       }else{
           $security_actions_list =  $security_actions->selectAll();
       }
       if(!$security_actions_list){
           $security_actions_list = new classsecurity_actions;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'security_actions', 'read', $retriveData->id_security_actions);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"security_actions":'. json_encode($security_actions_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    $security_actions = new classsecurity_actions;   
    $security_actions->action = $retriveData->action;
    $security_actions->id_security_actions = $retriveData->id_security_actions;
    $OPCODE_RESULT = $security_actions->update($security_actions,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'security_actions', 'update', $retriveData->id_security_actions);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $security_actions = new classsecurity_actions;   
      $QUERY_RESULT = $security_actions->delete($retriveData->id_security_actions,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'security_actions', 'delete', $retriveData->id_security_actions);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
