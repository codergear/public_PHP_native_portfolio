...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $contracts_payments->transaction_id = $retriveData->transaction_id;
        $contracts_payments->contract = $retriveData->contract;
     $OPCODE_RESULT = $contracts_payments->insert($contracts_payments,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_contracts_payments = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'contracts_payments', 'create', $retriveData->id_contracts_payments);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $contracts_payments = new classcontracts_payments();     
       if ($retriveData->id_contracts_payments != 0){
           $contracts_payments_list = $contracts_payments->selectBy('id_contracts_payments', $retriveData->id_contracts_payments);
           $contracts_payments_list = $contracts_payments_list ? $contracts_payments_list[0] : null ;
       }else{
           $contracts_payments_list =  $contracts_payments->selectAll();
       }
       if(!$contracts_payments_list){
           $contracts_payments_list = new classcontracts_payments;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'contracts_payments', 'read', $retriveData->id_contracts_payments);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"contracts_payments":'. json_encode($contracts_payments_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
