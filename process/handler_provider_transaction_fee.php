...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $provider_transaction_fee->transaction_fee = $retriveData->transaction_fee;
     $OPCODE_RESULT = $provider_transaction_fee->insert($provider_transaction_fee,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_provider_transaction_fee = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'provider_transaction_fee', 'create', $retriveData->id_provider_transaction_fee);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $provider_transaction_fee = new classprovider_transaction_fee();     
       if ($retriveData->id_provider_transaction_fee != 0){
           $provider_transaction_fee_list = $provider_transaction_fee->selectBy('id_provider_transaction_fee', $retriveData->id_provider_transaction_fee);
           $provider_transaction_fee_list = $provider_transaction_fee_list ? $provider_transaction_fee_list[0] : null ;
       }else{
           $provider_transaction_fee_list =  $provider_transaction_fee->selectAll();
       }
       if(!$provider_transaction_fee_list){
           $provider_transaction_fee_list = new classprovider_transaction_fee;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'provider_transaction_fee', 'read', $retriveData->id_provider_transaction_fee);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"provider_transaction_fee":'. json_encode($provider_transaction_fee_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    $provider_transaction_fee = new classprovider_transaction_fee;   
    $provider_transaction_fee->transaction_fee = $retriveData->transaction_fee;
    $provider_transaction_fee->id_provider_transaction_fee = $retriveData->id_provider_transaction_fee;
    $OPCODE_RESULT = $provider_transaction_fee->update($provider_transaction_fee,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'provider_transaction_fee', 'update', $retriveData->id_provider_transaction_fee);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $provider_transaction_fee = new classprovider_transaction_fee;   
      $QUERY_RESULT = $provider_transaction_fee->delete($retriveData->id_provider_transaction_fee,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'provider_transaction_fee', 'delete', $retriveData->id_provider_transaction_fee);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
