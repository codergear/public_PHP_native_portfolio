...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $doctors_status->status = $retriveData->status;
     $OPCODE_RESULT = $doctors_status->insert($doctors_status,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_doctors_status = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'doctors_status', 'create', $retriveData->id_doctors_status);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $doctors_status = new classdoctors_status();     
       if ($retriveData->id_doctors_status != 0){
           $doctors_status_list = $doctors_status->selectBy('id_doctors_status', $retriveData->id_doctors_status);
           $doctors_status_list = $doctors_status_list ? $doctors_status_list[0] : null ;
       }else{
           $doctors_status_list =  $doctors_status->selectAll();
       }
       if(!$doctors_status_list){
           $doctors_status_list = new classdoctors_status;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'doctors_status', 'read', $retriveData->id_doctors_status);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"doctors_status":'. json_encode($doctors_status_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    if (in_array($retriveData->id_doctors_status, [1,2,3])) {
       http_response_code(400);
    ...
    $doctors_status = new classdoctors_status;   
    $doctors_status->status = $retriveData->status;
    $doctors_status->id_doctors_status = $retriveData->id_doctors_status;
    $OPCODE_RESULT = $doctors_status->update($doctors_status,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'doctors_status', 'update', $retriveData->id_doctors_status);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
       ...
       // security ID read_only
      $doctors_status = new classdoctors_status;   
      $QUERY_RESULT = $doctors_status->delete($retriveData->id_doctors_status,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'doctors_status', 'delete', $retriveData->id_doctors_status);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
