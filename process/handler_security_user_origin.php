...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $security_user_origin->origin = $retriveData->origin;
     $OPCODE_RESULT = $security_user_origin->insert($security_user_origin,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_security_user_origin = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'security_user_origin', 'create', $retriveData->id_security_user_origin);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $security_user_origin = new classsecurity_user_origin();     
       if ($retriveData->id_security_user_origin != 0){
           $security_user_origin_list = $security_user_origin->selectBy('id_security_user_origin', $retriveData->id_security_user_origin);
           $security_user_origin_list = $security_user_origin_list ? $security_user_origin_list[0] : null ;
       }else{
           $security_user_origin_list =  $security_user_origin->selectAll();
       }
       if(!$security_user_origin_list){
           $security_user_origin_list = new classsecurity_user_origin;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'security_user_origin', 'read', $retriveData->id_security_user_origin);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"security_user_origin":'. json_encode($security_user_origin_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    if (in_array($retriveData->id_security_user_origin, [1,2,3])) {
       http_response_code(400);
    ...
    $security_user_origin = new classsecurity_user_origin;   
    $security_user_origin->origin = $retriveData->origin;
    $security_user_origin->id_security_user_origin = $retriveData->id_security_user_origin;
    $OPCODE_RESULT = $security_user_origin->update($security_user_origin,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'security_user_origin', 'update', $retriveData->id_security_user_origin);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
       ...
       // security ID read_only
      $security_user_origin = new classsecurity_user_origin;   
      $QUERY_RESULT = $security_user_origin->delete($retriveData->id_security_user_origin,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'security_user_origin', 'delete', $retriveData->id_security_user_origin);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
