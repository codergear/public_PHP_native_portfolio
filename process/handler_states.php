...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $states->state_name = $retriveData->state_name;
        $states->state_code = $retriveData->state_code;
     $OPCODE_RESULT = $states->insert($states,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_states = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'states', 'create', $retriveData->id_states);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $states = new classstates();     
       if ($retriveData->id_states != 0){
           $states_list = $states->selectBy('id_states', $retriveData->id_states);
           $states_list = $states_list ? $states_list[0] : null ;
       }else{
           $states_list =  $states->selectAll();
       }
       if(!$states_list){
           $states_list = new classstates;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'states', 'read', $retriveData->id_states);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"states":'. json_encode($states_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    $states->state_name = $retriveData->state_name;
    $states->state_code = $retriveData->state_code;
    $states->id_states = $retriveData->id_states;
    $OPCODE_RESULT = $states->update($states,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'states', 'update', $retriveData->id_states);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $states = new classstates;   
      $QUERY_RESULT = $states->delete($retriveData->id_states,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'states', 'delete', $retriveData->id_states);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
