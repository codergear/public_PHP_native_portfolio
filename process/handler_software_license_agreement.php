...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $software_license_agreement->document_content = $retriveData->document_content;
     $OPCODE_RESULT = $software_license_agreement->insert($software_license_agreement,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_software_license_agreement = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'software_license_agreement', 'create', $retriveData->id_software_license_agreement);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $software_license_agreement = new classsoftware_license_agreement();     
       if ($retriveData->id_software_license_agreement != 0){
           $software_license_agreement_list = $software_license_agreement->selectBy('id_software_license_agreement', $retriveData->id_software_license_agreement);
           $software_license_agreement_list = $software_license_agreement_list ? $software_license_agreement_list[0] : null ;
       }else{
           $software_license_agreement_list =  $software_license_agreement->selectAll();
       }
       if(!$software_license_agreement_list){
           $software_license_agreement_list = new classsoftware_license_agreement;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'software_license_agreement', 'read', $retriveData->id_software_license_agreement);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"software_license_agreement":'. json_encode($software_license_agreement_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    $software_license_agreement = new classsoftware_license_agreement;   
    $software_license_agreement->document_content = $retriveData->document_content;
    $software_license_agreement->id_software_license_agreement = $retriveData->id_software_license_agreement;
    $OPCODE_RESULT = $software_license_agreement->update($software_license_agreement,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'software_license_agreement', 'update', $retriveData->id_software_license_agreement);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $software_license_agreement = new classsoftware_license_agreement;   
      $QUERY_RESULT = $software_license_agreement->delete($retriveData->id_software_license_agreement,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'software_license_agreement', 'delete', $retriveData->id_software_license_agreement);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
