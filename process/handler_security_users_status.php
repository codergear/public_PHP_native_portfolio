...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $security_users_status->status = $retriveData->status;
     $OPCODE_RESULT = $security_users_status->insert($security_users_status,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_security_users_status = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'security_users_status', 'create', $retriveData->id_security_users_status);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $security_users_status = new classsecurity_users_status();     
       if ($retriveData->id_security_users_status != 0){
           $security_users_status_list = $security_users_status->selectBy('id_security_users_status', $retriveData->id_security_users_status);
           $security_users_status_list = $security_users_status_list ? $security_users_status_list[0] : null ;
       }else{
           $security_users_status_list =  $security_users_status->selectAll();
       }
       if(!$security_users_status_list){
           $security_users_status_list = new classsecurity_users_status;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'security_users_status', 'read', $retriveData->id_security_users_status);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"security_users_status":'. json_encode($security_users_status_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    if (in_array($retriveData->id_security_users_status, [1,2,3])) {
       http_response_code(400);
    ...
    $security_users_status = new classsecurity_users_status;   
    $security_users_status->status = $retriveData->status;
    $security_users_status->id_security_users_status = $retriveData->id_security_users_status;
    $OPCODE_RESULT = $security_users_status->update($security_users_status,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'security_users_status', 'update', $retriveData->id_security_users_status);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
       ...
       // security ID read_only
      $security_users_status = new classsecurity_users_status;   
      $QUERY_RESULT = $security_users_status->delete($retriveData->id_security_users_status,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'security_users_status', 'delete', $retriveData->id_security_users_status);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
