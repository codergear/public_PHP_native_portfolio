...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $groups_status->status = $retriveData->status;
     $OPCODE_RESULT = $groups_status->insert($groups_status,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_groups_status = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'groups_status', 'create', $retriveData->id_groups_status);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $groups_status = new classgroups_status();     
       if ($retriveData->id_groups_status != 0){
           $groups_status_list = $groups_status->selectBy('id_groups_status', $retriveData->id_groups_status);
           $groups_status_list = $groups_status_list ? $groups_status_list[0] : null ;
       }else{
           $groups_status_list =  $groups_status->selectAll();
       }
       if(!$groups_status_list){
           $groups_status_list = new classgroups_status;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'groups_status', 'read', $retriveData->id_groups_status);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"groups_status":'. json_encode($groups_status_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    if (in_array($retriveData->id_groups_status, [1,2,3])) {
       http_response_code(400);
    ...
    $groups_status = new classgroups_status;   
    $groups_status->status = $retriveData->status;
    $groups_status->id_groups_status = $retriveData->id_groups_status;
    $OPCODE_RESULT = $groups_status->update($groups_status,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'groups_status', 'update', $retriveData->id_groups_status);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
       ...
       // security ID read_only
      $groups_status = new classgroups_status;   
      $QUERY_RESULT = $groups_status->delete($retriveData->id_groups_status,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'groups_status', 'delete', $retriveData->id_groups_status);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
