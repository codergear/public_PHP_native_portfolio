...
function create($retriveData,$id_user) {
  ...
  try {
        $security_sections = new classsecurity_sections;
        $security_sections->action = $retriveData->action;
     $OPCODE_RESULT = $security_sections->insert($security_sections,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_security_sections = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'security_sections', 'create', $retriveData->id_security_sections);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $security_sections = new classsecurity_sections();     
       if ($retriveData->id_security_sections != 0){
           $security_sections_list = $security_sections->selectBy('id_security_sections', $retriveData->id_security_sections);
           $security_sections_list = $security_sections_list ? $security_sections_list[0] : null ;
       }else{
           $security_sections_list =  $security_sections->selectAll();
       }
       if(!$security_sections_list){
           $security_sections_list = new classsecurity_sections;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'security_sections', 'read', $retriveData->id_security_sections);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"security_sections":'. json_encode($security_sections_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    $security_sections = new classsecurity_sections;   
    $security_sections->action = $retriveData->action;
    $security_sections->id_security_sections = $retriveData->id_security_sections;
    $OPCODE_RESULT = $security_sections->update($security_sections,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'security_sections', 'update', $retriveData->id_security_sections);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $security_sections = new classsecurity_sections;   
      $QUERY_RESULT = $security_sections->delete($retriveData->id_security_sections,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'security_sections', 'delete', $retriveData->id_security_sections);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
