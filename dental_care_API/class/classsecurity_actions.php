...
class classsecurity_actions{
    public $action;
    public $id_security_actions = 0;
    public $_user_umo;
    public $_date_umo;

    function insert($security_actions,$id_user) {
        $conn = new classConexion();
        $sql = "CALL CRUD_security_actions(
            '".$security_actions->action."',
            '".$id_user."',
            '0',
            '',
            '1'
    ...
    function update($security_actions,$id_user){ 
        $conn = new classConexion();  
        $sql = "CALL CRUD_security_actions(
            '".$security_actions->action."',
            '".$id_user."',
            '".$security_actions->id_security_actions."',
            '',
            '3'
        )"; 
        $conn->Consulta($sql); 
        $last_id = $security_actions->id_security_actions;
        return true;
    } 
