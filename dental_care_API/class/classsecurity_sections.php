...
class classsecurity_sections{
    public $action;
    public $id_security_sections = 0;
    public $_user_umo;
    public $_date_umo;

    function insert($security_sections,$id_user) {
        $conn = new classConexion();
        $sql = "CALL CRUD_security_sections(
            '".$security_sections->action."',
            '".$id_user."',
            '0',
            '',
            '1'
    ...
    function update($security_sections,$id_user){ 
        $conn = new classConexion();  
        $sql = "CALL CRUD_security_sections(
            '".$security_sections->action."',
            '".$id_user."',
            '".$security_sections->id_security_sections."',
            '',
            '3'
        )"; 
        $conn->Consulta($sql); 
        $last_id = $security_sections->id_security_sections;
        return true;
    } 
