...
class classsecurity_roles{
    public $name;
    public $id_security_roles = 0;
    public $_user_umo;
    public $_date_umo;

    function insert($security_roles,$id_user) {
        $conn = new classConexion();
        $sql = "CALL CRUD_security_roles(
            '".$security_roles->name."',
            '".$id_user."',
            '0',
            '',
    ...
    function update($security_roles,$id_user){ 
        $conn = new classConexion();  
        $sql = "CALL CRUD_security_roles(
            '".$security_roles->name."',
            '".$id_user."',
            '".$security_roles->id_security_roles."',
            '',
            '3'
        )"; 
        $conn->Consulta($sql); 
        $last_id = $security_roles->id_security_roles;
        return true;
    } 
