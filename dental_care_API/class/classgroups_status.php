...
class classgroups_status{
    public $status;
    public $id_groups_status = 0;
    public $_user_umo;
    public $_date_umo;

    function insert($groups_status,$id_user) {
        $conn = new classConexion();
        $sql = "CALL CRUD_groups_status(
    ...
    function update($groups_status,$id_user){ 
        $conn = new classConexion();  
        $sql = "CALL CRUD_groups_status(
            '".$groups_status->status."',
            '".$id_user."',
            '".$groups_status->id_groups_status."',
            '',
            '3'
        )"; 
        $conn->Consulta($sql); 
        $last_id = $groups_status->id_groups_status;
        return true;
    } 
