...
class classpayment_methods{
    public $payment_method;
    public $id_payment_methods = 0;
    public $_user_umo;
    public $_date_umo;

    function insert($payment_methods,$id_user) {
        $conn = new classConexion();
        $sql = "CALL CRUD_payment_methods(
    ...
    function update($payment_methods,$id_user){ 
        $conn = new classConexion();  
        $sql = "CALL CRUD_payment_methods(
            '".$payment_methods->payment_method."',
            '".$id_user."',
            '".$payment_methods->id_payment_methods."',
            '',
            '3'
        )"; 
        $conn->Consulta($sql); 
        $last_id = $payment_methods->id_payment_methods;
        return true;
    } 
