...
class classdoctors_status{
    public $status;
    public $id_doctors_status = 0;
    public $_user_umo;
    public $_date_umo;

    function insert($doctors_status,$id_user) {
        $conn = new classConexion();
        $sql = "CALL CRUD_doctors_status(
    ...
    function update($doctors_status,$id_user){ 
        $conn = new classConexion();  
        $sql = "CALL CRUD_doctors_status(
            '".$doctors_status->status."',
            '".$id_user."',
            '".$doctors_status->id_doctors_status."',
            '',
            '3'
        )"; 
        $conn->Consulta($sql); 
        $last_id = $doctors_status->id_doctors_status;
        return true;
    } 
