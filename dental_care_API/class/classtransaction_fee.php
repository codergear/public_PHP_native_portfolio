...
class classtransaction_fee{
    public $transaction_fee;
    public $id_transaction_fee = 0;
    public $_user_umo;
    public $_date_umo;

    function insert($transaction_fee,$id_user) {
        $conn = new classConexion();
        $sql = "CALL CRUD_transaction_fee(
    ...
    function update($transaction_fee,$id_user){ 
        $conn = new classConexion();  
        $sql = "CALL CRUD_transaction_fee(
            '".$transaction_fee->transaction_fee."',
            '".$id_user."',
            '".$transaction_fee->id_transaction_fee."',
            '',
            '3'
        )"; 
        $conn->Consulta($sql); 
        $last_id = $transaction_fee->id_transaction_fee;
        return true;
    } 
