...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $payment_methods->payment_method = $retriveData->payment_method;
     $OPCODE_RESULT = $payment_methods->insert($payment_methods,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_payment_methods = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'payment_methods', 'create', $retriveData->id_payment_methods);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $payment_methods = new classpayment_methods();     
       if ($retriveData->id_payment_methods != 0){
           $payment_methods_list = $payment_methods->selectBy('id_payment_methods', $retriveData->id_payment_methods);
           $payment_methods_list = $payment_methods_list ? $payment_methods_list[0] : null ;
       }else{
           $payment_methods_list =  $payment_methods->selectAll();
       }
       if(!$payment_methods_list){
           $payment_methods_list = new classpayment_methods;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'payment_methods', 'read', $retriveData->id_payment_methods);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"payment_methods":'. json_encode($payment_methods_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    if (in_array($retriveData->id_payment_methods, [1,2,3])) {
       http_response_code(400);
    ...
    $payment_methods = new classpayment_methods;   
    $payment_methods->payment_method = $retriveData->payment_method;
    $payment_methods->id_payment_methods = $retriveData->id_payment_methods;
    $OPCODE_RESULT = $payment_methods->update($payment_methods,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'payment_methods', 'update', $retriveData->id_payment_methods);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
       ...
       // security ID read_only
      $payment_methods = new classpayment_methods;   
      $QUERY_RESULT = $payment_methods->delete($retriveData->id_payment_methods,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'payment_methods', 'delete', $retriveData->id_payment_methods);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
