...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $plans_status->status = $retriveData->status;
     $OPCODE_RESULT = $plans_status->insert($plans_status,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_plans_status = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'plans_status', 'create', $retriveData->id_plans_status);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $plans_status = new classplans_status();     
       if ($retriveData->id_plans_status != 0){
           $plans_status_list = $plans_status->selectBy('id_plans_status', $retriveData->id_plans_status);
           $plans_status_list = $plans_status_list ? $plans_status_list[0] : null ;
       }else{
           $plans_status_list =  $plans_status->selectAll();
       }
       if(!$plans_status_list){
           $plans_status_list = new classplans_status;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'plans_status', 'read', $retriveData->id_plans_status);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"plans_status":'. json_encode($plans_status_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    if (in_array($retriveData->id_plans_status, [1,2,3])) {
       http_response_code(400);
    ...
    $plans_status = new classplans_status;   
    $plans_status->status = $retriveData->status;
    $plans_status->id_plans_status = $retriveData->id_plans_status;
    $OPCODE_RESULT = $plans_status->update($plans_status,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'plans_status', 'update', $retriveData->id_plans_status);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
       ...
       // security ID read_only
      $plans_status = new classplans_status;   
      $QUERY_RESULT = $plans_status->delete($retriveData->id_plans_status,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'plans_status', 'delete', $retriveData->id_plans_status);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
