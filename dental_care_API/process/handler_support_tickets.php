...
function create($retriveData,$id_user) {
  ...
  try {
        $support_tickets = new classsupport_tickets;
        $support_tickets->tickets_user = $retriveData->tickets_user;
        $support_tickets->tickets_status = $retriveData->tickets_status;
        $support_tickets->tickets_date = $retriveData->tickets_date;
        $support_tickets->description = $retriveData->description;
     $OPCODE_RESULT = $support_tickets->insert($support_tickets,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_support_tickets = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'support_tickets', 'create', $retriveData->id_support_tickets);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $support_tickets = new classsupport_tickets();     
       if ($retriveData->id_support_tickets != 0){
           $support_tickets_list = $support_tickets->selectBy('id_support_tickets', $retriveData->id_support_tickets);
           $support_tickets_list = $support_tickets_list ? $support_tickets_list[0] : null ;
       }else{
           $support_tickets_list =  $support_tickets->selectAll();
       }
       $security_users=new classsecurity_users();
       $security_users=$security_users->selectAll();
       if (($support_tickets_list) && ($retriveData->id_support_tickets == 0)){
           foreach ($support_tickets_list as $support_tickets_item){
               foreach ($security_users as $item){
                   if ($item->id_security_users == $support_tickets_item->tickets_user){
                       $support_tickets_item->_tickets_user = $item->full_name;
                   }
               }
               if (!isset($support_tickets_item->_tickets_user)){$support_tickets_item->_tickets_user='';}
           }
       } 
       if(!$support_tickets_list){
           $support_tickets_list = new classsupport_tickets;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'support_tickets', 'read', $retriveData->id_support_tickets);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"support_tickets":'. json_encode($support_tickets_list).',"security_users":'. json_encode($security_users).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    $support_tickets->tickets_date = $retriveData->tickets_date;
    $support_tickets->description = $retriveData->description;
    $support_tickets->id_support_tickets = $retriveData->id_support_tickets;
    $OPCODE_RESULT = $support_tickets->update($support_tickets,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'support_tickets', 'update', $retriveData->id_support_tickets);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $support_tickets = new classsupport_tickets;   
      $QUERY_RESULT = $support_tickets->delete($retriveData->id_support_tickets,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'support_tickets', 'delete', $retriveData->id_support_tickets);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
