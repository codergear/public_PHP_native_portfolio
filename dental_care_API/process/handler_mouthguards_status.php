...
function create($retriveData,$id_user) {
  ...
  try {
        $mouthguards_status = new classmouthguards_status;
        $mouthguards_status->status = $retriveData->status;
     $OPCODE_RESULT = $mouthguards_status->insert($mouthguards_status,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_mouthguards_status = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'mouthguards_status', 'create', $retriveData->id_mouthguards_status);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $mouthguards_status = new classmouthguards_status();     
       if ($retriveData->id_mouthguards_status != 0){
           $mouthguards_status_list = $mouthguards_status->selectBy('id_mouthguards_status', $retriveData->id_mouthguards_status);
           $mouthguards_status_list = $mouthguards_status_list ? $mouthguards_status_list[0] : null ;
       }else{
           $mouthguards_status_list =  $mouthguards_status->selectAll();
       }
       if(!$mouthguards_status_list){
           $mouthguards_status_list = new classmouthguards_status;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'mouthguards_status', 'read', $retriveData->id_mouthguards_status);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"mouthguards_status":'. json_encode($mouthguards_status_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    if (in_array($retriveData->id_mouthguards_status, [1,2,3])) {
       http_response_code(400);
    ...
    $mouthguards_status = new classmouthguards_status;   
    $mouthguards_status->status = $retriveData->status;
    $mouthguards_status->id_mouthguards_status = $retriveData->id_mouthguards_status;
    $OPCODE_RESULT = $mouthguards_status->update($mouthguards_status,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'mouthguards_status', 'update', $retriveData->id_mouthguards_status);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
       ...
       // security ID read_only
      $mouthguards_status = new classmouthguards_status;   
      $QUERY_RESULT = $mouthguards_status->delete($retriveData->id_mouthguards_status,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'mouthguards_status', 'delete', $retriveData->id_mouthguards_status);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
