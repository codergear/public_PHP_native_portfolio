...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $method_of_contact->method_of_contact = $retriveData->method_of_contact;
     $OPCODE_RESULT = $method_of_contact->insert($method_of_contact,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_method_of_contact = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'method_of_contact', 'create', $retriveData->id_method_of_contact);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $method_of_contact = new classmethod_of_contact();     
       if ($retriveData->id_method_of_contact != 0){
           $method_of_contact_list = $method_of_contact->selectBy('id_method_of_contact', $retriveData->id_method_of_contact);
           $method_of_contact_list = $method_of_contact_list ? $method_of_contact_list[0] : null ;
       }else{
           $method_of_contact_list =  $method_of_contact->selectAll();
       }
       if(!$method_of_contact_list){
           $method_of_contact_list = new classmethod_of_contact;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'method_of_contact', 'read', $retriveData->id_method_of_contact);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"method_of_contact":'. json_encode($method_of_contact_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    ...
    if (in_array($retriveData->id_method_of_contact, [1,2,3])) {
       http_response_code(400);
    ...
    $method_of_contact = new classmethod_of_contact;   
    $method_of_contact->method_of_contact = $retriveData->method_of_contact;
    $method_of_contact->id_method_of_contact = $retriveData->id_method_of_contact;
    $OPCODE_RESULT = $method_of_contact->update($method_of_contact,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'method_of_contact', 'update', $retriveData->id_method_of_contact);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
       ...
       // security ID read_only
      $method_of_contact = new classmethod_of_contact;   
      $QUERY_RESULT = $method_of_contact->delete($retriveData->id_method_of_contact,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'method_of_contact', 'delete', $retriveData->id_method_of_contact);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
