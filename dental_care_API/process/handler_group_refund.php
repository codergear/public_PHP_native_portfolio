...
function read($retriveData,$id_user) {
   ...
   try {
       ...
       if ($retriveData->id_group_refund != 0){
           $group_refund_list = $group_refund->selectBy('id_group_refund', $retriveData->id_group_refund);
           $group_refund_list = $group_refund_list ? $group_refund_list[0] : null ;
       }else{
           $group_refund_list =  $group_refund->selectAll();
       }
       $groups=new classgroups();
       $groups=$groups->selectAll();
       if (($group_refund_list) && ($retriveData->id_group_refund == 0)){
           foreach ($group_refund_list as $group_refund_item){
               foreach ($groups as $item){
                   if ($item->id_groups == $group_refund_item->group_detail){
                       $group_refund_item->_group_detail = $item->entity_name;
                   }
               }
               if (!isset($group_refund_item->_group_detail)){$group_refund_item->_group_detail='';}
           }
       } 
       if(!$group_refund_list){
           $group_refund_list = new classgroup_refund;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'group_refund', 'read', $retriveData->id_group_refund);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"group_refund":'. json_encode($group_refund_list).',"groups":'. json_encode($groups).'}}';
