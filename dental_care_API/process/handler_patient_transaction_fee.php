...
function create($retriveData,$id_user) {
  ...
  try {
        $patient_transaction_fee = new classpatient_transaction_fee;
        $patient_transaction_fee->transaction_fee = $retriveData->transaction_fee;
     $OPCODE_RESULT = $patient_transaction_fee->insert($patient_transaction_fee,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_patient_transaction_fee = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'patient_transaction_fee', 'create', $retriveData->id_patient_transaction_fee);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $patient_transaction_fee = new classpatient_transaction_fee();     
       if ($retriveData->id_patient_transaction_fee != 0){
           $patient_transaction_fee_list = $patient_transaction_fee->selectBy('id_patient_transaction_fee', $retriveData->id_patient_transaction_fee);
           $patient_transaction_fee_list = $patient_transaction_fee_list ? $patient_transaction_fee_list[0] : null ;
       }else{
           $patient_transaction_fee_list =  $patient_transaction_fee->selectAll();
       }
       if(!$patient_transaction_fee_list){
           $patient_transaction_fee_list = new classpatient_transaction_fee;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'patient_transaction_fee', 'read', $retriveData->id_patient_transaction_fee);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"patient_transaction_fee":'. json_encode($patient_transaction_fee_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    $patient_transaction_fee = new classpatient_transaction_fee;   
    $patient_transaction_fee->transaction_fee = $retriveData->transaction_fee;
    $patient_transaction_fee->id_patient_transaction_fee = $retriveData->id_patient_transaction_fee;
    $OPCODE_RESULT = $patient_transaction_fee->update($patient_transaction_fee,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'patient_transaction_fee', 'update', $retriveData->id_patient_transaction_fee);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $patient_transaction_fee = new classpatient_transaction_fee;   
      $QUERY_RESULT = $patient_transaction_fee->delete($retriveData->id_patient_transaction_fee,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'patient_transaction_fee', 'delete', $retriveData->id_patient_transaction_fee);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
