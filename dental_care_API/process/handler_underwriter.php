...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $underwriter->underwriter = $retriveData->underwriter;
     $OPCODE_RESULT = $underwriter->insert($underwriter,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_underwriter = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'underwriter', 'create', $retriveData->id_underwriter);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $underwriter = new classunderwriter();     
       if ($retriveData->id_underwriter != 0){
           $underwriter_list = $underwriter->selectBy('id_underwriter', $retriveData->id_underwriter);
           $underwriter_list = $underwriter_list ? $underwriter_list[0] : null ;
       }else{
           $underwriter_list =  $underwriter->selectAll();
       }
       if(!$underwriter_list){
           $underwriter_list = new classunderwriter;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'underwriter', 'read', $retriveData->id_underwriter);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"underwriter":'. json_encode($underwriter_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    $underwriter = new classunderwriter;   
    $underwriter->underwriter = $retriveData->underwriter;
    $underwriter->id_underwriter = $retriveData->id_underwriter;
    $OPCODE_RESULT = $underwriter->update($underwriter,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'underwriter', 'update', $retriveData->id_underwriter);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $underwriter = new classunderwriter;   
      $QUERY_RESULT = $underwriter->delete($retriveData->id_underwriter,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'underwriter', 'delete', $retriveData->id_underwriter);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
