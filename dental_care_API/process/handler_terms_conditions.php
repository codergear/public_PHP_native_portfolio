...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $terms_conditions->terms_conditions_eng = $retriveData->terms_conditions_eng;
     $OPCODE_RESULT = $terms_conditions->insert($terms_conditions,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_terms_conditions = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'terms_conditions', 'create', $retriveData->id_terms_conditions);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $terms_conditions = new classterms_conditions();     
       if ($retriveData->id_terms_conditions != 0){
           $terms_conditions_list = $terms_conditions->selectBy('id_terms_conditions', $retriveData->id_terms_conditions);
           $terms_conditions_list = $terms_conditions_list ? $terms_conditions_list[0] : null ;
       }else{
           $terms_conditions_list =  $terms_conditions->selectAll();
       }
       if(!$terms_conditions_list){
           $terms_conditions_list = new classterms_conditions;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'terms_conditions', 'read', $retriveData->id_terms_conditions);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"terms_conditions":'. json_encode($terms_conditions_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ($_SERVER['SERVER_NAME'] == 'localhost'){
...
function update($retriveData,$id_user) {
   ...
   try {
    $terms_conditions = new classterms_conditions;   
    $terms_conditions->terms_conditions_eng = $retriveData->terms_conditions_eng;
    $terms_conditions->id_terms_conditions = $retriveData->id_terms_conditions;
    $OPCODE_RESULT = $terms_conditions->update($terms_conditions,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'terms_conditions', 'update', $retriveData->id_terms_conditions);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $terms_conditions = new classterms_conditions;   
      $QUERY_RESULT = $terms_conditions->delete($retriveData->id_terms_conditions,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'terms_conditions', 'delete', $retriveData->id_terms_conditions);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
