...
function create($retriveData,$id_user) {
  ...
  try {
        ...
        $mouthguards_manufacturer->manufacturer = $retriveData->manufacturer;
     $OPCODE_RESULT = $mouthguards_manufacturer->insert($mouthguards_manufacturer,$id_user);
     if($OPCODE_RESULT){
        $retriveData->id_mouthguards_manufacturer = $OPCODE_RESULT;
        Functions::security_access_log($id_user, 'mouthguards_manufacturer', 'create', $retriveData->id_mouthguards_manufacturer);
        http_response_code(200);
...
function read($retriveData,$id_user) {
   ...
   try {
       $mouthguards_manufacturer = new classmouthguards_manufacturer();     
       if ($retriveData->id_mouthguards_manufacturer != 0){
           $mouthguards_manufacturer_list = $mouthguards_manufacturer->selectBy('id_mouthguards_manufacturer', $retriveData->id_mouthguards_manufacturer);
           $mouthguards_manufacturer_list = $mouthguards_manufacturer_list ? $mouthguards_manufacturer_list[0] : null ;
       }else{
           $mouthguards_manufacturer_list =  $mouthguards_manufacturer->selectAll();
       }
       if(!$mouthguards_manufacturer_list){
           $mouthguards_manufacturer_list = new classmouthguards_manufacturer;
       }else{
       }
       if ($retriveData->id_affiliates_status != '-1') {
          Functions::security_access_log($id_user, 'mouthguards_manufacturer', 'read', $retriveData->id_mouthguards_manufacturer);
       }
       http_response_code(200);
       echo '{"message":"Successful","status":200,"data":{"mouthguards_manufacturer":'. json_encode($mouthguards_manufacturer_list).'}}';
   } catch (\Throwable | \Exception $e) { 
      Functions::security_access_log($id_user, basename(__FILE__). ' '.str_replace('\\', '.', __FUNCTION__).  ' exception', $e->getMessage(), 0);
      http_response_code(500);
      if ((Functions::get_enviroment() != 'production') && (Functions::get_enviroment() != 'sandbox')){
...
function update($retriveData,$id_user) {
   ...
   try {
    $mouthguards_manufacturer = new classmouthguards_manufacturer;   
    $mouthguards_manufacturer->manufacturer = $retriveData->manufacturer;
    $mouthguards_manufacturer->id_mouthguards_manufacturer = $retriveData->id_mouthguards_manufacturer;
    $OPCODE_RESULT = $mouthguards_manufacturer->update($mouthguards_manufacturer,$id_user);
        if($OPCODE_RESULT){
        Functions::security_access_log($id_user, 'mouthguards_manufacturer', 'update', $retriveData->id_mouthguards_manufacturer);
            http_response_code(200);
            echo '{"message":"Successful","status":200,"data":[]}';
        }else{
            http_response_code(500);
...
function delete($retriveData,$id_user) {
   ...
   try {
      $mouthguards_manufacturer = new classmouthguards_manufacturer;   
      $QUERY_RESULT = $mouthguards_manufacturer->delete($retriveData->id_mouthguards_manufacturer,$id_user);
      if($QUERY_RESULT->error_flag === false ){
        Functions::security_access_log($id_user, 'mouthguards_manufacturer', 'delete', $retriveData->id_mouthguards_manufacturer);
         http_response_code(200);
         echo '{"message":"Successful","status":200,"data":[]}';
      }else{
         http_response_code(500);
